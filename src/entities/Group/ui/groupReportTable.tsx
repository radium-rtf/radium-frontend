...
export const GroupReportTable: FC<CourseReportProps> = ({ courseReport }) => {
  ...
  const createSubtext = (text: string) => (
    <div className='mt-[0.38rem] text-[0.625rem] text-text-secondary'>
      {text}
    </div>
  );
  ...
  return (
    <Table
      headerRow={[
        <TableHeaderCell key='Имя' className='w-full text-sm text-text-primary'>
          <p>Имя</p>
        </TableHeaderCell>,
        <TableHeaderCell key='Итог' className='text-center'>
          <div className='m-auto w-full text-sm text-text-primary'>
            <p>Итог</p>
          </div>
        ...
        ...courseReport.header.values
          ...
          .map((section) => (
            <TableHeaderCell key={section.name} className='text-center'>
              <div
                className={cn(
                  'm-auto text-center text-sm text-text-primary',
              ...
              {createSubtext(
                (section.isModule ? 'глава' : '') + ' / ' + section.maxScore
              )}
            </TableHeaderCell>
          )),
      ]}
