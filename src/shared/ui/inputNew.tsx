...
const Input = forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    return (
      <div
        className='relative h-10 rounded-[0.5rem] text-[#B3B3B3]'
        style={style}
      >
        <input
          id={id || innerId}
          ...
          type={type}
          className={cn(
            'peer absolute inset-0 flex h-10 w-full rounded-[0.5rem] border border-white/10 bg-transparent pb-0.5 pl-[var(--left-padding)] pr-[var(--right-padding)] pt-3 text-sm text-foreground ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:select-none placeholder:text-transparent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50',
            className
          )}
          ref={ref}
          {...props}
