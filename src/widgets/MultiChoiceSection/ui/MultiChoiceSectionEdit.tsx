      ...
      >
        <form className='flex flex-col gap-4' onSubmit={handleSubmit(onSubmitHandler)}>
          ...
          <main>
            <ul className='flex flex-col gap-4'>
              {fields.map((field, index) => {
                return (
                  <div key={field.id} id={field.id} className='flex items-center gap-4'>
                    <Checkbox
                    ...
                    <Input
                      ...
                      {...register(`multichoice.variants.${index}.value`, {
                        onChange: (e) => {
                          ...
                          ).checked && setValue(`multichoice.answer.${index}`, e.target.value);
          ...
          <footer className='flex items-center gap-4 place-self-end'>
            ...
            <Button className='w-64 shrink-0' color='outlined' type='submit' disabled={!isValid}>
              <Icon type='save' className='text-inherit' />
              <span className='ml-[calc(50%-34px)] -translate-x-1/2'>Сохранить</span>
            </Button>
          </footer>
          {errors.multichoice?.variants && <p>{errors.multichoice.variants.message}</p>}
        </form>
      </Card>
    </>
  );
