...
export const FileSectionEdit: FC<FileSectionEditProps> = ({ sectionData }) => {
  // Edit setup
  ...
  const {
    ...
    defaultValues: {
      file: {
        ...
        fileTypes: sectionData.fileTypes,
      },
  ...
  const [updateFileAnswerSection] = useUpdateCourseFileSectionMutation();
      ...
      <form onSubmit={handleSubmit(onSubmitHandler)}>
        ...
        {isEditing && (
          <>
            ...
            <CardContent>
              <Input
                placeholder='Разрешенные расширения файлов (например, .txt) через запятую'
                {...register('file.fileTypes')}
              />
            </CardContent>
          </>
        )}
            ...
            >
              ...
              <span className='ml-[calc(50%-18px)] -translate-x-1/2'>
                {(isSubmitting && 'Сохраняем...') ||
                  (!isValid &&
                    (errors.root?.message ||
                      errors.file?.question?.message ||
                      errors.file?.fileTypes?.message ||
